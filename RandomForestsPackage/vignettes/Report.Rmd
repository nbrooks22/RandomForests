...
data_class2 <- list(x = matrix(c(X1,X2),nrow = 2, byrow = TRUE), y = f(X1,X2,e))
```
...
`bagging()` ist eine Erweiterung auf dem `greedy_cart()`, der mithilfe von Bootstrap Aggregation mehrere Trainingsbäume erstellt um bessere Vorhersagen treffen zu können. Wir wollen bei jedem Aufruf der Funktion `b` verschiedene Bäume erstellen.

```{r}
data <- bagging(data = data_reg2, type = "reg")
```

`data` ist eine Liste, wobei `data$Bagged_Trees` die Liste der Entscheidungsbäume (jeweils im Format von den `greedy_cart()` returns) ausgibt, und `data$Prediction` ein Vector von predictions ist. Die Anzahl von ausgegebenen Predictions ist gleich der Anzahl von eingegebenen Schätzdaten.
...
Die Anzahl von Bags die erstellt werden soll. Der default wert ist 5.
...
Eine Matrix mit den werten die man schätzen will, wo jede spalte einen Vektor von gleicher Dimension wie die x-werte der Eingangsdaten darstellt (in diesem Fall testen wir 3 verschiedene eindimensionale Werte).
...
Die Abbruchbedingungen zu jedem Baum sind von den default werten von `greedy_cart()` genommen.

Analog funktioniert das gierige Verfahren für Klassifikationsprobleme.

```{r}
data <- bagging(data = data_class2, b = 8, type = "class")
```
...
`make_prediction()` ist eine Funktion die erlaubt, unsere erstellten Bäume auch zu benutzen. Wir wollen den oder die Bäume durchlaufen, und jeweils schauen bei welchem c_value wir landen.

#### `tree_list`

Eine Liste von Environments, so wie sie von z.b. `greedy_cart()`, `bagging()` oder ähnlichem generiert werden. ACHTUNG: Muss im List() format übergeben werden, auch wenn es nur ein Baum hat
...
Eine Matrix mit den werten die man schätzen will, wo jede spalte einen Vektor von gleicher Dimension wie die x-werte der Eingangsdaten darstellt (in diesem Fall testen wir 4 verschiedene eindimensionale Werte).

```{r}
xlist <- matrix(c(0.1, 0.4, 0.6, 0.2), ncol = 2)
```

#### `type`

Entweder `type` = "reg" oder `type` = "class", was auch immer die entsprechenden Bäume für Daten haben.

```{r}
make_prediction(treelist$Bagged_Trees, xlist, "reg")
```
